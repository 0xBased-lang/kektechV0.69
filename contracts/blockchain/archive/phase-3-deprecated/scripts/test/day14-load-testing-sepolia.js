...
async function main() {
  ...
  log(`Creating 3 markets rapidly...`, "yellow");
  ...
  for (let i = 0; i < 3; i++) {
    try {
      ...
      log(`\n${colors.bright}Market ${i + 1}/3${colors.reset}`);
      log(`   Creating...`);
  ...
  log(`✅ Markets Created: ${results.marketsCreated.length}/3 (${(results.marketsCreated.length / 3 * 100).toFixed(0)}%)`);
  log(`⚡ Average Gas: ${Math.floor(results.avgGas).toLocaleString()}`);
  ...
  const successRate = results.marketsCreated.length / 3 * 100;
  if (successRate === 100) {
  ...
  if (results.marketsCreated.length === 3 && results.errors.length === 0) {
