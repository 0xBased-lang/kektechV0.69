...
describe("🔒 Security Fixes Validation - BasedAI Fork", function () {
    ...
    before(async function () {
        ...
        console.log("1️⃣  Deploying MasterRegistry...");
        const MasterRegistry = await ethers.getContractFactory("MasterRegistry");
        registry = await MasterRegistry.deploy(TX_OPTIONS);
        await registry.waitForDeployment();
        console.log("   ✅ MasterRegistry:", await registry.getAddress());
        ...
        // Register contracts
        console.log("6️⃣  Registering contracts in MasterRegistry...");
        await registry.setContract(ethers.id("AccessControlManager"), await accessControl.getAddress(), TX_OPTIONS);
        await registry.setContract(ethers.id("ParameterStorage"), await paramStorage.getAddress(), TX_OPTIONS);
        await registry.setContract(ethers.id("FlexibleMarketFactory"), await factory.getAddress(), TX_OPTIONS);
        await registry.setContract(ethers.id("ResolutionManager"), await resolutionManager.getAddress(), TX_OPTIONS);
