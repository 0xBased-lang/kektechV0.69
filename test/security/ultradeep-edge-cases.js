...
describe("🧠 ULTRADEEP SECURITY ANALYSIS", function () {
    ...
    before(async function () {
        ...
        registry = await ethers.getContractAt("MasterRegistry", deployedAddresses.MasterRegistry);
    ...
    describe("🔢 INTEGER OVERFLOW/UNDERFLOW EDGE CASES", function () {
        it("Should handle fee calculation with MAX_UINT256 bet amounts", async function () {
            ...
            await expect(
                market.connect(attacker).placeBet(0, { value: hugeBet })
    ...
    describe("🥪 MEV & FRONT-RUNNING PROTECTION", function () {
        it("Should handle rapid consecutive bets (sandwich attack scenario)", async function () {
            ...
            // Front-run
            await market.connect(attacker).placeBet(0, { value: attackerBet });

            // User transaction
            await market.connect(user1).placeBet(0, { value: userBet });

            // Back-run
            await market.connect(attacker).placeBet(1, { value: attackerBet });
    ...
    describe("💰 ECONOMIC ATTACK VECTORS", function () {
        ...
        it("Should prevent flash loan attacks", async function () {
            ...
            await market.connect(user1).placeBet(0, { value: largeBet });
    ...
    describe("⚡ FEE EXTRACTION EDGE CASES", function () {
        it("Should handle fee rounding errors across many transactions", async function () {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                await market.connect(user1).placeBet(i % 2, { value: betAmount });
                totalBets += betAmount;
            }
    ...
    describe("🚨 EMERGENCY SCENARIOS", function () {
        it("Should handle market with all funds in one outcome", async function () {
            ...
            await market.connect(user1).placeBet(0, { value: ethers.parseEther("0.1") });
            await market.connect(user2).placeBet(0, { value: ethers.parseEther("0.2") });
            await market.connect(user3).placeBet(0, { value: ethers.parseEther("0.15") });
